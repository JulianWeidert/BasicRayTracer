...
namespace brt {
	...
	lm::Vector3f Dielectric::refract(const lm::Vector3f& inDir, const lm::Vector3f& normal, float refractionRatio) const {
		auto cos = std::min((-inDir) * normal, 1.0f);

		auto dirPerp = refractionRatio * (inDir + cos * normal);
		auto dirPar = -std::sqrt(std::abs(1 - dirPerp * dirPerp)) * normal;

		auto outDir = dirPar + dirPerp;
		return outDir;
	}
	...
	std::optional<std::pair<Ray, lm::Vector3f>> Dielectric::scatter(const Ray& inRay, const HitResult& hitResult) {
		...
		// || this->reflectance(cosTheta, refractionRatio) > this->randomFloat()
